      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <main className="p-6 sm:p-8 space-y-5 flex-1 overflow-y-auto bg-[#0e0e0e] text-neutral-200">
          ...
          <div className="bg-[#141414] border border-white/10 rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            {/* Combined Bar + Line Visualization matching exact screenshot layout */}
            <div className="pt-4 space-y-3">
              <div className="relative h-64 flex items-end justify-between gap-3 px-8 pt-6 border-b border-white/10">
                {/* Left Y-axis label */}
                <span className="absolute -left-6 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] text-neutral-400 font-medium tracking-wider">
                  Przychód (zł)
                </span>

                {/* Right Y-axis label */}
                <span className="absolute -right-6 top-1/2 -translate-y-1/2 rotate-90 text-[10px] text-neutral-400 font-medium tracking-wider">
                  Zamówienia
                </span>

                {/* Y-axis left markers (Przychód) */}
                <div className="absolute left-2 top-0 bottom-6 flex flex-col justify-between text-[10px] text-neutral-400 font-mono pointer-events-none">
                  <span>350</span>
                  <span>300</span>
                  <span>250</span>
                  <span>200</span>
                ...
                <div className="absolute right-2 top-0 bottom-6 flex flex-col justify-between text-[10px] text-neutral-400 font-mono pointer-events-none">
                  <span>5.0</span>
                  <span>4.5</span>
                  <span>4.0</span>
                  <span>3.5</span>
                ...
                <svg className="absolute inset-x-8 top-6 bottom-6 w-[calc(100%-4rem)] h-[calc(100%-3rem)] pointer-events-none z-20 overflow-visible" viewBox="0 0 1000 200" preserveAspectRatio="none">
                  {chartMode === "Hours" ? (
                    <>
                      <path
              ...
              {/* X-axis Labels */}
              <div className="flex justify-between text-[11px] text-neutral-400 font-mono px-8">
                {(chartMode === "Hours"
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead className="sticky top-0 z-10 bg-[#0e0e0e]">
                  <tr className="bg-[#0e0e0e] border-b border-white/10 text-neutral-400 text-[11px]">
                    <th className="py-3 px-4 font-semibold text-neutral-300">Dzień ↑</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Zamówienia zrealizowane ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Średnia liczba zamówień ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Udział w liczbie zamówień ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Przychód (zł) ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Średni przychód (zł) ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Udział w przychodzie ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Średnia wartość zamówienia (zł) ↑↓</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5 text-neutral-300 font-sans">
                  ...
                  <tr className="bg-white/5 font-bold text-white border-b border-white/10 sticky top-[37px] z-10">
                    <td className="py-3.5 px-4 text-white font-bold bg-[#141414]">Wszystkie dni</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">23</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">2.9</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">100%</td>
                    <td className="py-3.5 px-4 text-right font-bold text-[#f26522] bg-[#141414]">1,459.00 PLN</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">182.38 PLN</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">100%</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">63.43 PLN</td>
                  </tr>
                  ...
                  {currentDataset.dayRows.map((row) => (
                    <tr key={row.id} className="hover:bg-white/[0.03] transition-colors border-b border-white/5">
                      <td className="py-3 px-4 text-neutral-200 font-medium">{row.day}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.orders}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.avgOrders}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.shareOrders}</td>
                      <td className="py-3 px-4 text-right text-neutral-200 font-mono font-medium">{row.revenue}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.avgIncome}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.revenueShare}</td>
                      <td className="py-3 px-4 text-right text-neutral-200 font-mono font-medium">{row.avgOrderValue}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-[#141414] border border-white/10 rounded-2xl relative shadow-xl overflow-hidden mt-6">
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-[380px] no-scrollbar">
              <table className="w-full text-left border-collapse text-xs">
                <thead className="sticky top-0 z-10 bg-[#0e0e0e]">
                  <tr className="bg-[#0e0e0e] border-b border-white/10 text-neutral-400 text-[11px]">
                    <th className="py-3 px-4 font-semibold text-neutral-300">Godzina ↑</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Zamówienia zrealizowane ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Średnia liczba zamówień ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Udział w liczbie zamówień ↑↓</th>
                    <th className="py-3 px-4 font-semibold text-neutral-300 text-right">Przychód (zł) ↑↓</th>
                ...
                <tbody className="divide-y divide-white/5 text-neutral-300 font-sans">
                  {/* Total Row */}
                  <tr className="bg-white/5 font-bold text-white border-b border-white/10 sticky top-[37px] z-10">
                    <td className="py-3.5 px-4 text-white font-bold bg-[#141414]">Wszystkie godziny</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">{currentDataset.hourRows.reduce((a, r) => a + r.orders, 0)}</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">-</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">100%</td>
                    <td className="py-3.5 px-4 text-right font-bold text-[#f26522] bg-[#141414]">{currentDataset.totalRev.toFixed(2)} PLN</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">-</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">100%</td>
                    <td className="py-3.5 px-4 text-right font-bold text-white bg-[#141414]">-</td>
                  </tr>
                  ...
                  {currentDataset.hourRows.map((row) => (
                    <tr key={row.id} className="hover:bg-white/[0.03] transition-colors border-b border-white/5">
                      <td className="py-3 px-4 text-neutral-200 font-medium">{row.time}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.orders}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.avgOrders}</td>
                      <td className="py-3 px-4 text-right text-neutral-300 font-mono">{row.shareOrders}</td>
                      <td className="py-3 px-4 text-right text-neutral-200 font-mono font-medium">{row.revenue}</td>
