...
import { Bell, RefreshCw, ChevronDown, MapPin, Check, Menu, X } from "lucide-react";
...
export default function AdminHeader({
  ...
  return (
    <>
      <header className="bg-[#141414]/90 backdrop-blur-md border-b border-white/10 sticky top-0 z-40 px-4 sm:px-6 py-3.5 sm:py-4 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
        {/* Title & Live Status */}
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white md:hidden transition-colors"
              aria-label="Otwórz nawigację"
            >
              <Menu className="w-5 h-5 text-[#f26522]" />
            </button>
            <div>
              <h1 className="font-judson font-bold text-lg sm:text-2xl text-white tracking-wide">
                {getTabTitle(activeTab)}
              </h1>
              <p className="text-[11px] sm:text-xs text-neutral-400 flex items-center gap-1.5 sm:gap-2 mt-0.5">
                <span className="inline-block w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span>Na żywo • Aktualny stan</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          >
            <button
              ...
              className="w-full bg-[#0e0e0e] border border-white/15 hover:border-white/25 text-xs text-white rounded-xl px-3 sm:px-3.5 py-2.5 flex items-center justify-between focus:outline-none focus:border-[#f26522] font-medium shadow-sm transition-all"
            >
          ...
          {onRefresh && (
            <button
              onClick={onRefresh}
              className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-neutral-300 hover:text-white transition-all active:scale-95 shrink-0"
              title="Odśwież dane"
            >
          ...
          <button
            className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-neutral-300 hover:text-white relative transition-all active:scale-95 shrink-0"
            title="Powiadomienia"
          >
      ...
      {/* Mobile Mobile Navigation Drawer Overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          />
          <div className="relative z-10 w-72 max-w-[80vw] bg-[#141414] h-full shadow-2xl flex flex-col animate-slideRight">
            <button
              type="button"
              onClick={() => setMobileMenuOpen(false)}
              className="absolute top-4 right-4 z-20 p-2 bg-white/10 text-white rounded-full hover:bg-white/20"
              aria-label="Zamknij menu"
            >
              <X className="w-5 h-5" />
            </button>
            <div className="h-full overflow-y-auto [&>aside]:!flex [&>aside]:!w-full [&>aside]:!static [&>aside]:!h-full">
              <AdminSidebar />
            </div>
          </div>
        </div>
