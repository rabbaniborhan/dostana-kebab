...
export default function DonutChartCard({
  ...
  const list = segments || items || [];
  const mainText = centerText || centerValue || "";
  const subText = centerSubtext || centerLabel || "";
      ...
      <div className="flex items-center justify-around py-2">
        <div className="relative w-32 h-32 flex items-center justify-center">
          ...
          <div className="absolute flex flex-col items-center justify-center text-center">
            <span className="text-sm font-black text-white">{mainText}</span>
            {subText && (
              <span className="text-[9px] text-neutral-400 font-medium">{subText}</span>
            )}
          </div>
        </div>
